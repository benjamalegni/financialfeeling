...
import { Plus, Send, LogOut, User, BarChart3, Zap, Search, Star, TrendingUp, DollarSign, Bitcoin, Building2, X, ArrowLeft } from 'lucide-react'
...
export default function HomePage() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Si el usuario no está logueado y está escribiendo, redirigir al login
    if (!user && value.trim()) {
      router.push('/login')
      return
    }
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      // Si el usuario no está logueado y presiona Enter, redirigir al login
      if (!user) {
        e.preventDefault()
        router.push('/login')
        return
  ...
  const savePortfolio = async () => {
    ...
    try {
      ...
      alert(`Portfolio saved with ${selectedAssets.length} assets!`)
      setShowAssetSelector(false)
      setSelectedAssets([]) // Clear selection after saving
      // Reload user assets to show updated portfolio
  ...
  const removeAsset = async (symbol: string) => {
    ...
    try {
      ...
      // Update local state
      setSelectedAssets(prev => prev.filter(asset => asset !== symbol))
      console.log(`Asset ${symbol} removed successfully`)
    } catch (error) {
      console.error('Error removing asset:', error)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {/* Shared Sidebar */}
      <SharedSidebar selectedAssets={selectedAssets} />
      ...
      <div className="ml-16 p-8">
        <div className="max-w-6xl mx-auto">
                ...
                <div className="flex items-center space-x-3">
                  <div className="relative flex-1">
                    <Input
                      ...
                      placeholder={
                        user
                          ? "Type to select any asset or click + button to open asset selector..."
                          : "Type to start selecting assets (login required)"
                      }
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Dialog open={showAssetSelector} onOpenChange={setShowAssetSelector}>
                      ...
                      {user && (
                        <DialogContent className="bg-gray-900 border-gray-700 w-[95vw] h-[95vh] max-w-none max-h-none overflow-hidden">
                          <DialogHeader>
                            <div className="flex items-center justify-between">
                              ...
                              >
                                ...
                                Save & Close ({selectedAssets.length})
                              </Button>
                            </div>
                          ...
                          <div className="flex flex-col h-full space-y-4">
                            ...
                            {/* Assets Grid */}
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 flex-1 overflow-y-auto min-h-0">
                            ...
                            {selectedAssets.length > 0 && (
                              <div className="border-t border-gray-700 pt-4 bg-gray-800/50 rounded-lg p-3">
                                <h4 className="text-white font-semibold mb-2 flex items-center gap-2">
                                  <Star className="h-4 w-4 text-yellow-400" />
                                  Your Portfolio ({selectedAssets.length})
                                </h4>
                                <div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto">
                                  {selectedAssets.map(symbol => {
                                    const asset = financialAssets.find(a => a.symbol === symbol)
                                    return (
                                      <div key={symbol} className="flex items-center gap-1 bg-gray-700 rounded-full px-3 py-1">
                                        <Badge className="bg-blue-600 hover:bg-blue-700 border-0">
                                          {symbol}
                                        </Badge>
                                        <button
                                          onClick={() => removeAsset(symbol)}
                                          className="text-red-400 hover:text-red-300 hover:bg-red-900/20 p-1 rounded-full transition-colors ml-1"
                                          title={`Remove ${symbol}`}
                                        >
                                          <X className="h-3 w-3" />
                                        </button>
                            ...
                            {/* Action Buttons */}
                            <div className="flex justify-center items-center gap-4 pt-4 border-t border-gray-700 mt-auto">
                              <div className="text-sm text-gray-400">
                                {selectedAssets.length > 0 ? `${selectedAssets.length} assets selected` : 'No assets selected'}
                              </div>
                              <Button
                                variant="outline"
                                onClick={() => setShowAssetSelector(false)}
                                className="border-gray-600 text-gray-400 hover:bg-gray-700 hover:text-white px-6 py-2"
                              >
                                Cancel
                              </Button>
                            </div>
          ...
          {/* How to Use Section */}
          <div className="mb-8">
            <h2 className="text-xl font-semibold mb-6 text-center">How to Use Stock Analysis</h2>
